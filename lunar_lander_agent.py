...
class LunarLanderAgent:
    ...
    def __init__(self, env_config: dict, **kwargs):
        ...
        self.observation_as_rgb = self.env_config.get(
    ...
    def get_action(self, observation: np.ndarray) -> Union[int, np.ndarray]:
        ...
        env.close()
        pos_h, pos_v, v_h, v_vert, angle, v_ang, p_g, p_d = observation
        action = 0
        v_goals = -0.076
        goals = np.array([0, v_goals, 0])
        v = np.array([v_h, v_vert, v_ang])
        print(f'v : {v[1]} , v_goals : {goals[1]}')
        dif = v - goals
        print(dif[1])
        err = np.power(dif, 2)

        m = max(err)
        index = int(np.where(err == m)[0])
        #print(index)
...
if __name__ == '__main__':
    ...
    echelon_id: int = 3
    echelon_key = [key for key in configs.keys(
    ) if key.startswith(f"Echelon {echelon_id}")][0]
