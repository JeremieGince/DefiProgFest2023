...
class LunarLanderAgent:
	...
	def __init__(self, env_config: dict, **kwargs):
		...
		self.rn_gen_name = get_random_generators()[
			"continuous_random_gen" if self.env_config["continuous"] else "discrete_random_gen"
		]
		self.rn_gen = (
			get_random_continuous_action if self.env_config["continuous"] else get_random_discrete_action
		)
		self.mock_env = self.make_env()
	...
	def get_action(self, observation: np.ndarray) -> Union[int, np.ndarray]:
		...
		"""
		action = self.rn_gen(self.mock_env.action_space, self.rn_gen_name)
		return action
